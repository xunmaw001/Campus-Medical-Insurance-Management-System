<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','充值') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="chongzhijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'充值':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','充值') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="chongzhijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'充值':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','充值') && contents.tableBtnIcon == 0" type="success" size="mini" @click="chongzhijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'充值':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','报销到账') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="baoxiaodaozhangCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'报销到账':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','报销到账') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="baoxiaodaozhangCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'报销到账':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','报销到账') && contents.tableBtnIcon == 0" type="success" size="mini" @click="baoxiaodaozhangCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'报销到账':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','消费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xiaofeijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'消费':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','消费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xiaofeijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'消费':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','消费') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xiaofeijiluCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'消费':'' }}</el-button>
    ...
    <chongzhijilu-cross-add-or-update v-if="chongzhijiluCrossAddOrUpdateFlag" :parent="this" ref="chongzhijiluCrossaddOrUpdate"></chongzhijilu-cross-add-or-update>
    <baoxiaodaozhang-cross-add-or-update v-if="baoxiaodaozhangCrossAddOrUpdateFlag" :parent="this" ref="baoxiaodaozhangCrossaddOrUpdate"></baoxiaodaozhang-cross-add-or-update>
    <xiaofeijilu-cross-add-or-update v-if="xiaofeijiluCrossAddOrUpdateFlag" :parent="this" ref="xiaofeijiluCrossaddOrUpdate"></xiaofeijilu-cross-add-or-update>
...
import chongzhijiluCrossAddOrUpdate from "../chongzhijilu/add-or-update";
import baoxiaodaozhangCrossAddOrUpdate from "../baoxiaodaozhang/add-or-update";
import xiaofeijiluCrossAddOrUpdate from "../xiaofeijilu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      chongzhijiluCrossAddOrUpdateFlag: false,
      baoxiaodaozhangCrossAddOrUpdateFlag: false,
      xiaofeijiluCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    chongzhijiluCrossAddOrUpdate,
    baoxiaodaozhangCrossAddOrUpdate,
    xiaofeijiluCrossAddOrUpdate,
  },
  methods: {
    ...
    chongzhijiluCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.chongzhijiluCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.chongzhijiluCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.chongzhijiluCrossaddOrUpdate.init(row.id,type);
      });
    },
    baoxiaodaozhangCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.baoxiaodaozhangCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$storage.set('statusColumnName',statusColumnName);
      this.$storage.set('statusColumnValue',statusColumnValue);
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.baoxiaodaozhangCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.baoxiaodaozhangCrossaddOrUpdate.init(row.id,type);
      });
    },
    xiaofeijiluCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xiaofeijiluCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$storage.set('statusColumnName',statusColumnName);
      this.$storage.set('statusColumnValue',statusColumnValue);
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.xiaofeijiluCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.xiaofeijiluCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
